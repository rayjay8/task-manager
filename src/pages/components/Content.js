...
const Content = () => {
  ...
  useEffect(() => {
    if (id) {
      let endpoint = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/tasks?sort[0]=createdAt:desc`;
      if (sort === "date") {
        endpoint = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/tasks`;
      ...
      if (sort === "priority") {
        ...
        if (ascending) {
          endpoint = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/tasks?sort[0]=priority:desc`;
        } else {
          endpoint = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/tasks?sort[0]=priority:asc`;
        }
      }
  ...
  useEffect(() => {
    if (id) {
      fetch(
        // "http://localhost:1337/api/completed-tasks?sort[0]=createdAt:desc" +
        //   "&filters[userId][$eq]=" +
        //   id
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/completed-tasks?sort[0]=createdAt:desc&filters[userId][$eq]=${id}`
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  useEffect(() => {
    if (id) {
      fetch(
        // "http://localhost:1337/api/deleted" + "?filters[userId][$eq]=" + id
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/deleted?filters[userId][$eq]=${id}`
      )
        .then((response) => response.json())
        .then((data) => {
