...
const Charts = () => {
  ...
  useEffect(() => {
    if (id) {
      fetch(
        // "http://localhost:1337/api/tasks" + "?filters[userId][$eq]=" + id
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/tasks` +
          "?filters[userId][$eq]=" +
          id
      )
  ...
  useEffect(() => {
    if (id) {
      fetch(
        // "http://localhost:1337/api/completed-tasks" +
        //   "?filters[userId][$eq]=" +
        //   id
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/completed-tasks` +
          "?filters[userId][$eq]=" +
          id
      )
        .then((response) => response.json())
  ...
  useEffect(() => {
    if (id) {
      fetch(
        ...
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/deleted` +
          "?filters[userId][$eq]=" +
          id
      )
        .then((response) => response.json())
