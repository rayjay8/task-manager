...
const Header = ({ initialTime, showSidebar, setShowSidebar }) => {
  ...
  const [profile, setProfile] = useState(
    "/uploads/bouncy_user_icon_2_8abfc57901.png"
  );
  ...
  useEffect(() => {
    if (id) {
      fetch(`${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/users/${id}`)
  ...
  const [uploadedFile, setUploadedFile] = useState({
    filePath: "/uploads/bouncy_user_icon_2_8abfc57901.png",
  });
      ...
      <div className={styles.right}>
        {user ? (
          <div className={styles.profile}>
            {profile && (
              <img
                src={`${process.env.NEXT_PUBLIC_STRAPI_API_URL}${profile}`}
                alt="Profile Picture"
