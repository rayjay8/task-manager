...
const CategoryContent = () => {
  ...
  useEffect(() => {
    if (id) {
      let endpoint =
        // "http://localhost:1337/api/tasks?filters[category][$eq]=" +
        // category +
        // "&filters[userId][$eq]=" +
        // id +
        // "&sort[0]=createdAt:desc";
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/tasks?filters[category][$eq]=${category}&filters[userId][$eq]=${id}&sort[0]=createdAt:desc`;
      if (sort === "date") {
        endpoint =
          //   "http://localhost:1337/api/tasks?filters[category][$eq]=" + category;
          // endpoint += "&sort[0]=date";
          `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/tasks?filters[category][$eq]=${category}&filters[userId][$eq]=${id}&sort[0]=date`;
        if (ascending) {
          endpoint += ":asc";
        } else {
          endpoint += ":desc";
      ...
      if (sort === "priority") {
        endpoint =
          // "http://localhost:1337/api/tasks?filters[category][$eq]=" + category;
          `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/tasks?filters[category][$eq]=${category}`;
  ...
  useEffect(() => {
    if (id) {
      fetch(
        // "http://localhost:1337/api/completed-tasks?filters[category][$eq]=" +
        //   category +
        //   "&filters[userId][$eq]=" +
        //   id +
        //   "&sort[0]=createdAt:desc"
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/completed-tasks?filters[category][$eq]=${category}&filters[userId][$eq]=${id}&sort[0]=createdAt:desc`
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  useEffect(() => {
    if (id) {
      fetch(
        // "http://localhost:1337/api/deleted?filters[category][$eq]=" +
        //   category +
        //   "&filters[userId][$eq]=" +
        //   id
        `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/deleted?filters[category][$eq]=${category}&filters[userId][$eq]=${id}`
      )
        .then((response) => response.json())
        .then((data) => {
